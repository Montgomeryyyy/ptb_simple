...
POPULATION_PATH = "outputs/population_test_preterm.yaml.json"
ID_COL = "b_cpr"
OUT_PATH = "outputs/ptb_ehr_xgboost_risk_scores.csv"
DROP_FEATURE_COLS = {
    "m_cpr",
    "pregnancy_start",
    "pregnancy_end",
...
def main(tabular_ehr_path: str, img_preds_path: str=None, population_path: str=None):
    ...
    if population_path:
        ...
        if isinstance(population, dict):
            population_b_cprs = population.get("b_cprs", [])
        elif isinstance(population, list):
            population_b_cprs = population
        else:
            raise TypeError(f"Unexpected population type: {type(population)}")
        valid_b_cprs = list(set(img_b_cprs) & set(population_b_cprs))
    else:
        valid_b_cprs = img_b_cprs
    ...
    df_test = clean_df.filter(pl.col(ID_COL).is_in(list(valid_b_cprs)))
    df_train = clean_df.filter(~pl.col(ID_COL).is_in(list(valid_b_cprs)))
...
if __name__ == "__main__":
    main(PATH, IMG_PREDS, POPULATION_PATH)
